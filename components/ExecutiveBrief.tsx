...
const ExecutiveBrief: React.FC<ExecutiveBriefProps> = ({ data, alerts }) => {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const result = await generateExecutiveBrief(data, alerts);
      setBrief(result);
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg col-span-1 lg:col-span-2 border border-gray-700">
      <div className="flex justify-between items-center mb-4">
        ...
        {!brief && (
           ...
           disabled={loading}
           className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-md transition duration-200 disabled:opacity-50 flex items-center"
         >
           {loading ? (
             <>
